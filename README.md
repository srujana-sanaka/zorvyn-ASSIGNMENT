...
4. Add environment variables:
   ...
   - `CORS_ORIGIN=...`
5. Run the SQL in `sql/schema.sql` on your Render Postgres instance.

### Railway
1. Create a **New Project** → add **PostgreSQL** plugin.
2. Deploy the Node service.
3. Ensure `DATABASE_URL` is available (Railway provides it).
4. Set `JWT_SECRET`, `JWT_EXPIRES_IN`, `NODE_ENV=production`, `CORS_ORIGIN`.
5. Run `sql/schema.sql` in the Railway Postgres console.

### Vercel (backend guide)
Vercel is optimized for serverless functions; Express apps typically require adaptation.
Recommended approach for this assignment:
- Use Render/Railway for the Express API.
- Use Vercel for a frontend (if any), pointing to the deployed API base URL.

If you must deploy on Vercel:
- Convert the API to serverless functions (not included here to keep Express architecture clean).
